...
while run:
    ...
    if not finish:
        ...
        if level_1:
            ...
            if sprite.collide_rect(nevidimka_level_2, werewolf):
                werewolf.level_update(2)
            #barriers_1.draw(mw)
            strilka_lvl_1.reset()
            camerman.draw()
            camerman.moves()
            ...
            if sprite.collide_rect(colide_dialogue_3, camerman):
                ...
                if keys[K_SPACE] and d_2 == 0:
                    dokazi += 1
                    d_2 =1
            if sprite.collide_rect(car_colide, camerman) and keys[K_SPACE] and dokazi == 3:
                finish = True
        ...
        if level_2:
            ...
            if (((camerman.position == 0 and werewolf.position == 0) or (werewolf.position == 1 and camerman.position == 1))):
    ...
    if level_2 and sprite.collide_rect(camerman, werewolf):
        finish = True
        lose = transform.scale(image.load("Assets/game_over.jpg"), [1500, 900])
        mw.fill((255, 255, 255))
        mw.blit(lose, (0, 0))
